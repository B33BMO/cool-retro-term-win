...
#if defined(Q_OS_WIN)
    // Use QHostAddress on Windows (avoid inet_aton/in_addr)
    {
        QHostAddress addr;
        isIpAddress = addr.setAddress(_host);
    }
#else
...
// ═══════════════════════════════════════════════════════════════════
// Windows ProcessInfo implementation
// ═══════════════════════════════════════════════════════════════════
#if defined(Q_OS_WIN)
...
{
public:
    explicit WindowsProcessInfo(int pid, bool readEnvironment = false)
    ...
    bool readProcessInfo(int pid, bool readEnvironment) override
    {
        ...
        if (hProcess) {
            ...
            if (QueryFullProcessImageNameW(hProcess, 0, exeName, &size)) {
                QString fullPath = QString::fromWCharArray(exeName, size);
