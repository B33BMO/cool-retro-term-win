...
bool HistorySearch::search(int startColumn, int startLine, int endColumn, int endLine) {
    ...
    while ((blockSize = qMin(10000, linesToRead - linesRead)) > 0) {
        ...
        int matchStart;
        QRegularExpressionMatch qreMatch;
        if (m_forwards)
        {
            matchStart = string.indexOf(m_regExp, startColumn, &qreMatch);
            if (matchStart >= endPosition)
                matchStart = -1;
        }
        else
        {
            matchStart = string.lastIndexOf(m_regExp, endPosition - 1, &qreMatch);
        ...
        if (matchStart > -1)
        {
            int matchEnd = matchStart + qreMatch.capturedLength() - 1;
