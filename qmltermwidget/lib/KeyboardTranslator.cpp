...
QList<KeyboardTranslatorReader::Token> KeyboardTranslatorReader::tokenize(const QString& line)
{
    ...
    static const QRegularExpression titleRe(QRegularExpression::anchoredPattern(QLatin1String("keyboard\\s+\"(.*)\"")));
    // key line: key KeySequence : "output"
    // key line: key KeySequence : command
    static const QRegularExpression keyRe(QRegularExpression::anchoredPattern(QLatin1String("key\\s+([\\w\\+\\s\\-\\*\\.]+)\\s*:\\s*(\"(.*)\"|\\w+)")));
    ...
    QRegularExpressionMatch titleMatch = titleRe.match(text);
    QRegularExpressionMatch keyMatch = keyRe.match(text);
