...
ColorEntry ColorScheme::colorEntry(int index , uint randomSeed) const
{
    ...
    QRandomGenerator rng(randomSeed);
        ...
        !_randomTable[index].isNull() )
    {
        ...
        int hueDifference = range.hue ? (int)(rng.generate() % range.hue) - range.hue/2 : 0;
        int saturationDifference = range.saturation ? (int)(rng.generate() % range.saturation) - range.saturation/2 : 0;
        int  valueDifference = range.value ? (int)(rng.generate() % range.value) - range.value/2 : 0;
