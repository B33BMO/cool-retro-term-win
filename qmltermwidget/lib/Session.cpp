...
#if defined(Q_OS_WIN)
    ptySlaveFd = -1;  // Not applicable on Windows
#else
...
#if defined(Q_OS_WIN)
    // Windows shell detection: pwsh -> powershell -> cmd.exe
    if (exec.isEmpty()) {
        ...
        if (!pwsh.isEmpty()) {
            ...
            if (!ps.isEmpty()) {
                ...
                exec = QStandardPaths::findExecutable(QStringLiteral("cmd"));
                if (exec.isEmpty())
                    exec = QStringLiteral("cmd.exe");
            }
        }
    } else if (!QFile::exists(exec)) {
        // If specified program doesn't exist at that path, try finding it in PATH
        QString found = QStandardPaths::findExecutable(exec);
        if (!found.isEmpty())
            exec = found;
    }
#else
    // Unix shell detection (original logic)
...
// Provide a close() method that Pty can call on Windows
#if defined(Q_OS_WIN)
// Session::close() is already defined above
#endif
...
#if defined(Q_OS_WIN)
    Q_UNUSED(signal);
    // On Windows, we don't have Unix signals. Instead we rely on
    // the ConPTY being closed (which signals the child to exit).
    // The actual termination happens in Session::close() -> finished signal.
    return false;
#else
...
#if defined(Q_OS_WIN)
        // On Windows, ConPTY always reports normal exit
