...
void TerminalDisplay::setVTFont(const QFont& f)
{
  QFont font = f;
    ...
    // QFont::ForceIntegerMetrics removed in Qt 6
...
{
  // In Qt 6, use angleDelta().y() for vertical scrolling
  ...
  if ( _mouseMarks )
  {
    ...
    else
    {
        ...
        int key = deltaY > 0 ? Qt::Key_Up : Qt::Key_Down;

        // angleDelta gives rotation in eighths of a degree
        int wheelDegrees = deltaY / 8;
...
void TerminalDisplay::simulateWheel(int x, int y, int buttons, int modifiers, QPointF angleDelta){
    QWheelEvent event(QPointF(x,y), QPointF(x,y), QPoint(0,0), angleDelta.toPoint(),
