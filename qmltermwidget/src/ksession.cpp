...
#if defined(Q_OS_WIN)
    QString envshell = qEnvironmentVariable("COMSPEC");
    QString shellProg = envshell.isEmpty() ? QStringLiteral("cmd.exe") : envshell;
#else
    QString envshell = QString::fromLocal8Bit(qgetenv("SHELL"));
    QString shellProg = envshell.isEmpty() ? QStringLiteral("/bin/bash") : envshell;
#endif
...
{
#if defined(Q_OS_WIN)
    // On Windows, just send the cd command directly
    // The shell will handle it whether it's in the foreground or not
    QString cmd = "cd /d \"" + dir + "\"\r";
    sendText(cmd);
#else
    /*
